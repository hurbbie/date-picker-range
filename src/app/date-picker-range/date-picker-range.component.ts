...
export class DatePickerRangeComponent implements OnInit {
  ...
  @Input() getDateFromPicker: string = '';
  ...
  onDateActive(){
    this.active_popup = !this.active_popup
  }

  clearDate(){
    this.getDateFromPicker = 'เดือน'
  }

  onClick(indexClicked:any) {
    if (this.rangeIndexes[0] === 0) {
      this.rangeIndexes[0] = this.globalIndexOffset+indexClicked;
    } else
    if (this.rangeIndexes[1] === 0) {
